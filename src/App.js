...
class App extends Component {
  ...
  getTokenBalance = async () => {
    const { contract } = this.state;
    // Get the value of price
    var tokenBalance = await contract.methods
      .REFUbalanceOf(this.state.accounts[0])
      .call();
  ...
  buyTokens = async () => {
    ...
    try {
      ...
      var result = await contract.methods
        .CustomerBuyToken(this.state.BuytokenAmount)
        .send({
  ...
  createAsset = async () => {
    const { contract } = this.state;
    try {
      const casset = await contract.methods
        .createAsset(this.state.AssetSupply, this.state.AssetName)
  ...
  QueryAsset = async () => {
    ...
    try {
      const asset = await contract.methods
        .tokenizedAssets(this.state.AssetNum)
        .call();
      console.log(asset);
      this.setState({
        AssetSupply: Number(asset[1]),
        AssetRem: Number(asset[2]),
        AssetName: asset[3],
      });
    } catch (error) {
      console.error("Error creating asset:", error);
  ...
  LendToAsset = async () => {
    ...
    try {
      ...
      const asset = await contract.methods
        .CustomerBuyAsset(this.state.AssetNum, this.state.LendTokenAmount)
        .send({ from: this.state.accounts[0] });
      console.log(asset);

      const result = await contract.methods
        .fractionalOwnership(this.state.AssetNum, this.state.accounts[0])
        .call();
  ...
  emergencyStop = async () => {
    ...
    // Update state with the result.
    if (isStopped) this.setState({ isStopped: "Stopped" });
  };
  ...
  startService = async () => {
    ...
    // Update state with the result.
    if (!isStopped) this.setState({ isStopped: "Started" });
  };
  render() {
    if (!this.state.web3) {
    ...
    return (
      <div className="App">
          ...
          <div>
            ...
            <section className="Account-section">
              ...
              <div className="Context-information">
                ...
                {this.state.networkId !== 11155111 && (
                  <p id="inline">
                    This DAPP is currently working on GANACHE, please press the
                    button
                  </p>
                )}
                {this.state.networkId !== 11155111 && (
                  <button onClick={this.switchNetwork}>
                    Switch to Sepolia
                  </button>
                )}
              </div>
            ...
            <section className="RefuInfo-section">
              <div className="RefuToken-information">
                ...
                <div className="RefuToken-information-text"></div>
              </div>
            </section>
        ...
        <div className="RefuToken-actions">
          ...
          <p>
            {" "}
            <button id="button-send-price" onClick={this.getPrice}>
              Token Price
            </button>{" "}
          </p>
          {this.state.tokenPrice > 0 && (
            <p>
              <b>Token Price:</b> 1 Ether by {this.state.tokenPrice} RefuTokens
            </p>
          )}
          ...
          {this.state.tokenPrice > 0 && (
            <input
              placeholder="Insert RefuToken Quantity"
              onChange={(e) =>
                this.setState({
          ...
          {this.state.tokenPrice > 0 && (
            <button id="button-send" onClick={this.buyTokens}>
              Buy Tokens
            </button>
          )}
          {this.state.value && !isNaN(this.state.value) && (
            <p>
              You are going to buy {this.state.value} tokens using{" "}
              {this.state.value / this.state.tokenPrice} Ether. In Wei:{" "}
              {this.state.web3.utils.toWei(
                this.state.value / this.state.tokenPrice,
                "ether"
              )}
            </p>
          )}
          ...
          {
            <button id="button-send-RefuBalance" onClick={this.getTokenBalance}>
              Get RefuToken Balance
            </button>
          }
          ...
          {
            <p>
              Provide Total Supply for Apartment:
              <input
                placeholder="Total Supply of Tokens"
                onChange={(e) => this.setState({ AssetSupply: e.target.value })}
              ></input>
            </p>
          }
          {
            <p>
              Provide Name for Apartment:
              <input
          ...
          {
            <button id="button-create-aprtmnt" onClick={this.createAsset}>
              Create Apartment
            </button>
          }
          <br></br>
          ...
          {
            <p>
              Provide Apartment Number:
              <input
                placeholder="Number"
                onChange={(e) => this.setState({ AssetNum: e.target.value })}
              ></input>
            </p>
          }
          {
            <button id="button-query-aprtmnt" onClick={this.QueryAsset}>
              Query Apartment
            </button>
          }
          <br></br>
          {
            <p>
              Apartment Supply: <b>{this.state.AssetSupply}</b>
            </p>
          }
          {
            <p>
              Apartment Remaing: <b>{this.state.AssetRem}</b>
            </p>
          }
          {
            <p>
              Apartment Name: <b>{this.state.AssetName}</b>
            </p>
          }
          <br></br>
          ...
          {
            <p>
              Provide REFUTOKEN Amount to Lend:
              <input
                placeholder="NumberToLend"
                onChange={(f) =>
                  this.setState({ LendTokenAmount: f.target.value })
                }
              ></input>
            </p>
          }
          {
            <button id="button-send-lend" onClick={this.LendToAsset}>
              Lend to Apartment
            </button>
          }
          {this.state.AssetFractBalance != null && (
            <p>
              Your Refutoken balance lent to Apartment{" "}
              <b>{this.state.AssetNum}</b> is:{" "}
              <b>{this.state.AssetFractBalance}</b>
            </p>
          )}
          {/* Button to stop refutoken  */}
          <h3>Emergency Stop</h3>
          <button id="button-send" onClick={this.emergencyStop}>
            STOP REFU
          </button>
          <button id="button-send" onClick={this.startService}>
            START REFU
          </button>
          {
            <p>
              <b>Status:</b> {this.state.isStopped}
            </p>
          }
